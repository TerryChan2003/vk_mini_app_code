<template>
  ...
  >
    ...
    <List v-if="this.filteredFriends.length">
      <Cell v-for="friend in paginatedFriends" :key="friend.id" :indicator="friend.sex | getGenus">
        <template v-slot:before>
          <Avatar type="image" :src="friend.photo_200" />
        </template>
        {{friend.first_name}} {{friend.last_name}}
      </Cell>
      <infinite-loading @infinite="this.infiniteHandler">
...
export default {
  ...
  methods: {
    ...
    ...mapActions("friends", ["fetchFriends"])
  },
  filters: {
    getGenus(n) {
      switch (n) {
  ...
  computed: {
    ...mapGetters("friends", ["allFriends", "getStatus"]),
    filteredFriends() {
